...
- 会話の文脈（前提知識）: [${docContent}]`;
    ...
    if (status === 'Failed') {
       return (
         <div className="flex flex-col items-center justify-center h-[60vh] space-y-8 animate-fadeIn">
           ...
           <button onClick={onCancel} className="px-10 py-4 border border-[#1B4332] text-[#1B4332] font-bold uppercase tracking-widest text-xs hover:bg-white">Back to Home</button>
         </div>
       );
    }
    return (
      <div className="flex flex-col items-center justify-center h-[60vh] space-y-12 animate-fadeIn">
        <h2 className="text-2xl font-bold title-serif text-[#1B4332] tracking-widest uppercase">Initializing Neural Link</h2>
        <div className="w-48 h-48 border-2 border-[#D4AF37] rounded-full flex items-center justify-center relative">
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <span className="text-[10px] font-bold text-[#1B4332] uppercase tracking-widest animate-pulse">{status}</span>
          </div>
        </div>
      </div>
    );
  ...
  if (countdown !== null) {
    return (
      <div className="flex flex-col items-center justify-center h-[60vh] space-y-12 animate-fadeIn">
        <h2 className="text-4xl font-bold title-serif text-[#1B4332] tracking-widest">Prepare Yourself</h2>
        <div className="w-64 h-64 border-2 border-[#D4AF37] rounded-full flex items-center justify-center shadow-xl">
          <span className="text-8xl font-black title-serif text-[#1B4332] italic">{countdown}</span>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-10 animate-fadeIn relative">
      <div className="grid grid-cols-12 gap-6">
        <div className="col-span-8 deco-panel p-8 flex justify-between items-center corner-stepped">
          <div className="flex items-center space-x-6">
            <div className={`w-3 h-3 rounded-full bg-[#1B4332] ${sessionActive ? 'animate-ping' : ''}`}></div>
            <div>
              <span className="font-bold text-[#1B4332]/40 uppercase tracking-widest text-[9px]">Status</span>
              <p className="font-bold text-xl text-[#1B4332] title-serif">{status}</p>
            </div>
          </div>
          <div className="text-right">
             <span className="font-bold text-[#1B4332]/40 uppercase tracking-widest text-[9px]">Tier</span>
             <p className="font-bold text-xl text-[#D4AF37] title-serif">{level}</p>
          </div>
        </div>
        <div className="col-span-4 deco-panel p-8 bg-[#1B4332] text-[#D4AF37] border-none flex items-center justify-center corner-stepped">
          <div className="text-center">
            <span className="font-bold uppercase tracking-[0.3em] text-[9px]">Time</span>
            <div className="text-4xl font-bold title-serif tracking-tighter">{Math.floor(timeLeft/60)}:{timeLeft%60<10?'0':''}{timeLeft%60}</div>
          </div>
        </div>
      </div>
      ...
      <div className="flex justify-center">
        <button 
          ...
          className="w-full max-w-xl py-10 bg-[#1B4332] text-[#D4AF37] border-2 border-[#D4AF37] font-bold uppercase tracking-[0.4em] hover:bg-[#D4AF37] hover:text-[#1B4332] transition-all text-base shadow-2xl corner-stepped"
        >
      ...
      <div className="max-h-80 overflow-y-auto deco-panel p-8 text-sm bg-white/60 corner-stepped">
          <div className="flex justify-between items-center mb-6 border-b border-[#1B4332]/10 pb-4">
             <h4 className="text-[9px] font-bold text-[#1B4332] uppercase tracking-[0.4em]">Official Transcript History</h4>
          </div>
