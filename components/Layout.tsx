...
export const Layout: React.FC<LayoutProps> = ({ children, isAdmin, onToggleAdmin }) => {
  ...
  return (
    <div className="min-h-screen flex flex-col relative">
      <header className="bg-[#1B4332] text-white border-b-2 border-[#D4AF37] relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto px-6 py-10 flex justify-between items-center relative z-10">
          <div className="flex items-center space-x-6">
            <div className="relative">
              <div className="w-16 h-16 border-2 border-[#D4AF37] rounded-full flex items-center justify-center shadow-lg bg-[#1B4332]">
            ...
            <div>
              ...
              <p className="text-[10px] tracking-[0.6em] uppercase font-bold text-[#C0C0C0] mt-2">Akita RC 弐号機</p>
            </div>
          </div>
          ...
          {isAdmin && (
            <button 
              ...
              className="text-[10px] font-black border border-[#D4AF37] px-6 py-2 hover:bg-[#D4AF37] hover:text-[#1B4332] transition-all uppercase tracking-widest"
            >
              Exit Dashboard
            </button>
          )}
      ...
      <footer className="bg-[#1B4332] border-t-2 border-[#D4AF37] py-10 text-center relative overflow-hidden">
      ...
      {showPasswordPrompt && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-[#1B4332]/90 backdrop-blur-md animate-fadeIn">
          <div className="w-full max-w-md deco-panel p-10 shadow-2xl corner-stepped">
            <button 
              onClick={() => setShowPasswordPrompt(false)}
              className="absolute top-4 right-4 text-[#1B4332] font-bold text-xs hover:text-[#D4AF37]"
            >
              [CLOSE]
            </button>
            <div className="text-center space-y-8">
              <h3 className="text-2xl font-bold title-serif text-[#1B4332]">Access Required</h3>
              <form onSubmit={handlePasswordSubmit} className="space-y-6">
                <input
                  ...
                  className={`w-full px-4 py-4 border-b-2 text-center font-bold tracking-[0.4em] outline-none transition-all ${
                    isError ? 'border-red-500 bg-red-50' : 'border-[#1B4332] focus:border-[#D4AF37]'
                  }`}
                />
                <button 
                  type="submit"
